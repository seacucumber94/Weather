<?php  $scrpt_vrsn_dt  = 'fct_wu_popup_daily.php|01|2023-09-09|';  # PHP 8.2 graph | release 2012_lts
#-----------------------------------------------
#                                script settings
#-----------------------------------------------
...
'.$close.'
    <span class="tab" style="margin: 0 auto; font-size: 12px;">
        ...
        <label class="tablinks"          onclick="openTab(event, \'Text_lines\')">'   .lang('Texts').'</label>
    </span>
    <small style="float: right; padding-top: 0px;">
        <a href="https://www.wunderground.com/" target="_blank" style="color: grey">'
        .lang("Powered by").':&nbsp;WeatherUnderground&nbsp;&nbsp;</a>
        </small> 
</div>'.PHP_EOL;
...
$graphsNights   = array();
$today          = 0;
/*     [cloudCover] => Array [0] => 76
    [dayOrNight] => Array [0] => D
    [daypartName] => Array [0] => Today
    [iconCode] => Array [0] => 26
    [iconCodeExtend] => Array  [0] => 2600
    [narrative] => Array [0] => Cloudy. High 15ºC. Winds ESE and variable.
    [precipChance] => Array[0] => 2
    [precipType] => Array[0] => rain
    [qpf] => Array[0] => 0
    [qpfSnow] => Array[0] => 0
    [qualifierCode] => Array [0] => 
    [qualifierPhrase] => Array[0] =>  [2] => Areas of patchy fog early.
    [relativeHumidity] => Array [0] => 79
    [snowRange] => Array[0] => 
    [temperature] => Array [0] => 15
    [temperatureHeatIndex] => Array [0] => 15
    [temperatureWindChill] => Array[0] => 14
    [thunderCategory] => Array [0] => 
    [thunderIndex] => Array [0] => 0
    [uvDescription] => Array [0] => Low
    [uvIndex] => Array [0] => 1
    [windDirection] => Array [0] => 120
    [windDirectionCardinal] => Array [0] => ESE
    [windPhrase] => Array [0] => Winds ESE and variable.
    [windSpeed] => Array [0] => 6
    [wxPhraseLong] => Array [0] => Cloudy
    [wxPhraseShort] => Array[0] => */

# time day night 
$times  = $response['validTimeUtc'];
foreach ($times as $time)
     {  $sun_arr        = date_sun_info((int) $time, $lat, $lon);
        ...
        $timelocal	= $time + $utcDiff + 5*3600; //  has 07:00 for day and 19:00 for night
        if ($timelocal < $graphsStart) { $graphsStart= $timelocal;}
        if ($timelocal > $graphsStop)  { $graphsStop = $timelocal;}
        $arrTimeGraph[] = $timelocal;
        $arrTimeGraph[] = $timelocal+ 11*3600; 
        }
$graphsStop = $graphsStop + 11*3600;
#
# IMPORTANT  check incomplete values in the first item
$strtCnt= 0;   
$endCnt = count($parts['iconCode']);  
if (trim($parts['iconCode'][0])  == '')  // first icon empty => incomplete or passed daypart
     {  $strtCnt        = 1;
        $endCnt         = $endCnt - 1; # echo '<pre>'.print_r($parts['iconCode'],true); exit;
        array_shift($arrTimeGraph);
        $graphsStart    =  $graphsStart + 12*3600;}
if ($endCnt > 12)      
     {  $endCnt = 12;}
#
# iconcode
for ($n = $strtCnt; $n < $endCnt; $n++)  
     {  $icon           = $parts['iconCode'][$n];
        $icon           = $WU_icn_tr [$icon];
        $arrIconGraph[] = './pws_icons/'.$icon.'.svg';
        }                               #echo __LINE__.' arrIconGraph '.print_r($arrIconGraph,true); #exit;
# temperature $arrTempGraph  
for ($n = $strtCnt; $n < $endCnt; $n++)
     {  $temp           = $parts['temperature'][$n];
        if ($temp < $graphTempMin) { $graphTempMin = $temp;}
        if ($temp > $graphTempMax) { $graphTempMax = $temp;}    
        $arrTempGraph []= $temp;}                               
#                                       echo __LINE__.' arrTempGraph '.print_r($arrTempGraph,true); #exit;
# precipitation $arrRainGraph
for ($n = $strtCnt; $n < $endCnt; $n++)
     {  $rain   = $parts['qpf'][$n];
        $snow   = $parts['qpfSnow'][$n];
        $precip = $rain + $snow;
        if ($precip  > $graphRainMax)  {  $graphRainMax = $precip;}
        $arrRainGraph[] = $precip;}                               
#                                       echo __LINE__.' arrRainGraph '.print_r($arrRainGraph,true); #exit;
# wind gust $arrWindGraph
for ($n = $strtCnt; $n < $endCnt; $n++)
     {  $wind           = $parts['windSpeed'][$n];
        if ($wind  > $graphWindMax) { $graphWindMax = $wind;}
        $arrWindGraph[] = $wind;}                               
#                                       echo __LINE__.' arrWindGraph '.print_r($arrWindGraph,true); #exit;
# wind dir / icon $arrWdirGraph
for ($n = $strtCnt; $n < $endCnt; $n++)
     {  $wdeg   = $parts['windDirection'][$n];
        $compass= windlabel($wdeg);  // to make sure we have the english code
	if (strlen ($compass) > 3) 
	     {  $compass = substr ($compass,0,1);} 
	$arrWdirGraph[] = $compass;}                               
#                                       echo __LINE__.' arrWdirGraph '.print_r($arrWdirGraph,true); #exit;
#baro $arrBaroGraph  n/a for WU
#
# humid $arrHumGraph $graphHumMax
for ($n = $strtCnt; $n < $endCnt; $n++)
     {  $hum            = $parts['relativeHumidity'][$n];
        if ($hum  > $graphHumMax) { $graphHumMax = $hum;} 
        $arrHumGraph[]  = $hum;} 
#                                       echo __LINE__.' arrHumGraph '.print_r($arrHumGraph,true); #exit;
#
$count  = count($arrHumGraph);  
for ($n = 0; $n < $count; $n++)
     {  $graphsData .= 'tsv['.$n.'] ="'.
		$arrTimeGraph[$n].'|'.      # 0
                $arrTempGraph[$n].'|'.      # 1
		...
		$arrSolarGraph   .'|'.      # 10
		date('c',$time).'";'.PHP_EOL;	
	}	                       # echo __LINE__.' graphsData '.$graphsData; 
#
...
# ----------------- WU MOS TABLE
if (1 == 1) {
if     ($windunit == 'mph') { $toKnots = 0.868976;} 
elseif ($windunit == 'km/h'){ $toKnots = 0.5399568;} 
elseif ($windunit == 'm/s') { $toKnots = 1.943844;}
else                        { $toKnots = 1;}
#
echo '<table class= "tabcontent div_height  forecast" id="Forecast" 
        style=" width: 100%;  margin: 0px auto; text-align: center; border-spacing: 2px; background-color: transparent; color: black;">'.PHP_EOL;
$count  = $endCnt - $strtCnt; 
$fc_col_width   = round(100/$count,3,PHP_ROUND_HALF_DOWN);
#
# daypartName
$line   = '<tr class="fc_night font_head">';
for ($n = $strtCnt; $n < $endCnt; $n++)  
     {  $line  .= '<td style="width: '.$fc_col_width.'%;"><small>'.$parts['daypartName'][$n].'</small></td>'.PHP_EOL;}
echo $line.'</tr>'.PHP_EOL;
#
# iconCode
$line   = '<tr class="fc_day">';
for ($n = $strtCnt; $n < $endCnt; $n++)  
     {  if ($parts['dayOrNight'][$n] == 'D') 
             {  $fc_class       = 'fc_day';} 
        else {  $fc_class       = 'fc_night';} 
        $iconCd = $parts['iconCode'][$n];     
        $icon   = $icn_prefix.$WU_icn_tr [$iconCd].$icn_post;
        $url    = '<img src="'.$icon.'" class="fc_icon" alt="'.$iconCd.'">';
        $line  .= '<td class="'.$fc_class.'">'.$url.'</td>'.PHP_EOL;}
echo $line.'</tr>'.PHP_EOL;
#
# wxPhraseLong
$line   = '<tr class="fc_day">';
for ($n = $strtCnt; $n < $endCnt; $n++)  
     {  $line  .= '<td>'.$parts['wxPhraseLong'][$n].'</td>'.PHP_EOL;}
echo $line.'</tr>'.PHP_EOL;
#
# temperature
$line   = '<tr class="fc_day" style="text-shadow: black 1px 1px;font-weight: bolder;">';
for ($n = $strtCnt; $n < $endCnt; $n++)  
     {  $temp   = $parts['temperature'][$n];
        $line  .= '<td style="font-size: 20px; color: '.temp_color($temp).'">'.$temp.' &deg;'.$tempunit.'</td>'.PHP_EOL;}
echo $line.'</tr>'.PHP_EOL;
#
# UV uvDescription  
echo '<tr class="fc_night  font_head"><td class="" colspan="'.$count.'">'.lang('UV').'</td></tr>'.PHP_EOL;
# uvIndex
$data   = 'uvIndex';
$desc   = 'uvDescription';
$arr    = $parts['uvIndex'];  
$texts  = $parts['uvDescription']; #echo '<pre> uvDescription'.print_r($texts,true); exit;
$line   = '<tr class="fc_day">';
for ($n = $strtCnt; $n < $endCnt; $n++)  
     {  if ( $parts['dayOrNight'][$n]  == 'D' 
          && (float) $parts['uvIndex'][$n] <> 0)
             {  $value  = round($parts['uvIndex'][$n]);
                if ($value > 11) {$value = 11;}
                $return = '<div class="my_uv" style="background-color:'.$fll_uv[$value].'; ">'.$value.'</div>';
                $return.= $parts['uvDescription'][$n];}
        else {  $return = $nouv;} 
        $line  .= '<td>'.$return.'</td>'.PHP_EOL;} // eo uv
echo $line.'</tr>'.PHP_EOL;
#
# rain description
echo '<tr class="fc_night  font_head"><td class="" colspan="'.$count.'">'.lang('Precipitation').'<small> '.lang($rainunit).'</small></td></tr>'.PHP_EOL;
# rain
$total  = 0;
$line   = '<tr class="fc_day">';
for ($n = $strtCnt; $n < $endCnt; $n++)  
     {  $rain   = (float) $parts['qpf'][$n];
        $snow   = (float) $parts['qpfSnow'][$n];
        $amount = $rain + $snow;
        if ($amount == 0)
             {  $line  .= '<td>'.$norain.'</td>'.PHP_EOL;
                continue;}
        else {  $total  = $total + $amount;}
#
        $string = $extra = ''; # $content.'% - ';
        $type   = $parts['precipType'][$n];
        $change = $parts['precipChance'][$n];
        if ($type == 'snow')
             {  if ($snow > 0) 
                     {  $string .= $snow.$rainunit.' ';}
                $string .=  $change.'% '.$snowflakesvg;}
        else {  if ($rain > 0)     
                     {  $string .= $extra.$rain.$rainunit.' ';
                        $extra = '';}
                $string .= $extra.$change.'% '.$rainsvg;
                }
        $line  .= '<td>'.$string .'</td>'.PHP_EOL;
        }
if ($total == 0)
     {  echo '<tr class="fc_day"><td colspan = "'.$count.'">'
                .lang('No measurable rain expected during this period.').'</td></tr>'.PHP_EOL;}
else {  echo $line.'</tr>'.PHP_EOL;}
#
# Wind description
echo '<tr class="fc_night font_head"><td class="" colspan="'.$count.'">'.lang('Wind from').'</td></tr>'.PHP_EOL;
# wind
$wdegs  = $parts['windDirection'];
$wTxts  = $parts['windDirectionCardinal'];
$lineDg = '<tr class="fc_day">';
$lineTx = '<tr class="fc_day">';
for ($n = $strtCnt; $n < $endCnt; $n++)  
     {  $wdeg   = $parts['windDirection'][$n];
        $compass= windlabel((int) $wdeg );  // to make sure we have the english code
	if (strlen ($compass) > 3) 
	     {  $compass = substr ($compass,0,1);}
	$lineDg.= '<td><img src="img/windicons/'.$compass.'.svg" width="20" height="20"  alt="'.$compass.'"></td>'.PHP_EOL;
	$lineTx.= '<td>'.$parts['windDirectionCardinal'][$n].'</td>'.PHP_EOL;
	}
echo $lineDg.'</tr>'.PHP_EOL;
echo $lineTx.'</tr>'.PHP_EOL;

echo '<tr class="fc_night font_head"><td class="" colspan="'.$count.'">'.lang('Windspeed').'<small> '.lang($windunit).'</small></td></tr>'.PHP_EOL;
$lineSp = '<tr class="fc_day">';
$lineBt = '<tr class="fc_day">';
$lineBn = '<tr class="fc_day">';
for ($n = $strtCnt; $n < $endCnt; $n++)  
     {  $speed  = $parts['windSpeed'][$n];
        $knots  = round($speed*$toKnots,1);
        foreach ($lvl_bft as $key => $value)
             {  if ($knots > $value) {continue;}  # $key=12; # for test 
                break;}
        $bft_nr = $key;  
        $lineSp.= '<td>'.$speed.'</td>'.PHP_EOL;
        $lineBt.= '<td>'.lang($bft_txt[$key]).'</td>'.PHP_EOL;
        $lineBn.= '<td>'.$bft_nr.lang('Bft').'</td>'.PHP_EOL;
        }
echo $lineSp.'</tr>'.PHP_EOL;
echo $lineBt.'</tr>'.PHP_EOL;
echo $lineBn.'</tr>'.PHP_EOL;
/*
#
for ($n = $start; $n < $rows; $n++) // print 1 row / daypart with all data in coloms
     {  echo '<tr style="border-bottom: 1px grey solid; ">'; 
        if ($parts['dayOrNight'][$n] == 'D')   { $color = $clrwrm; } else { $color = $clrcld; }
                ...
                switch ($key){
                    ...
                    case 'iconCode':
                     ...
                     case 'wxPhraseLong': 
                        ...
                        $string = $parts['qualifierPhrase'][$n];
                    ...
                    case 'temperatureHeatIndex':
                        ...
                        $chill  = $parts['temperatureWindChill'][$n];
                    ...
                    case 'precipChance': 
                        ...
                        $string = ''; # $content.'% - ';
                        if ($type == 'snow')
                             {  if ((float)$parts['qpfSnow'][$n] > 0) { $string .= $parts['qpfSnow'][$n].$rainunit.' ';}
                                $string .=  $content.'% '.$snowflakesvg;}
                        else {  if ((float)$parts['qpf'][$n] > 0)     { $string .= $parts['qpf'][$n].$rainunit.' ';}
                    ...
                    case 'windDirection': 
                        ...
                        echo PHP_EOL.'<td>';
                        echo '<img src="img/windicons/'.$compass.'.svg" width="20" height="20" alt="'.$parts['windDirectionCardinal'][$n].'"  style="vertical-align: bottom;"> ';
                        echo $parts['windDirectionCardinal'][$n];
                        break;
                    case 'uvIndex': 
                        echo PHP_EOL.'<td>';
                        $value  = trim($content);
...
if (1 == 1) {
#
echo '<table class= "tabcontent div_height font_head" id="Text_lines" '
        .'style=" width: 100%; height: 100%; margin: 0px auto; text-align: center; border-collapse: collapse;">'.PHP_EOL;
#
for ($n = $strtCnt; $n < $endCnt; $n++)  
     {  if ($parts['dayOrNight'][$n] == 'D')   
              { $color = $clrwrm; } 
        else  { $color = $clrcld; }
        $string = str_replace (' ','&nbsp;',$parts['daypartName'][$n]);
        echo '<tr>
...
.$parts['narrative'][$n].'</td></tr>'.PHP_EOL;
} // eo rows
echo '</table>'.PHP_EOL;
...
';
        $return         .= '    </style>'.PHP_EOL;        return $return;
 }
